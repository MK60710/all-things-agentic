...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(`${apiUrl.replace(/\/$/, "")}/clarifications/${encodeURIComponent(id)}/answer`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not answer this question" }, { status: response.status });
  } catch (error) {
