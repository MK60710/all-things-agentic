...
export async function GET(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(`${apiUrl.replace(/\/$/, "")}/sessions/${encodeURIComponent(id)}/paper-map`, {
      headers: {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not map paper connections" }, { status: response.status });
  } catch (error) {
