...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not load sessions" }, { status: response.status });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const data = await response.json();
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not create the session" }, { status: response.status });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Backend unavailable" }, { status: 502 });
  }
}
