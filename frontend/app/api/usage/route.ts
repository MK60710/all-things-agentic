...
export async function GET(request: NextRequest) {
  ...
  try {
    const response = await fetch(`${apiUrl.replace(/\/$/, "")}/usage`, {
      headers: {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not load usage" }, { status: response.status });
  } catch (error) {
