...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(`${apiUrl.replace(/\/$/, "")}/papers/${encodeURIComponent(id)}/status`, {
      headers: {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not load paper status" }, { status: response.status });
  } catch (error) {
