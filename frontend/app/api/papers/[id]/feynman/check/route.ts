...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const response = await fetch(`${apiUrl.replace(/\/$/, "")}/papers/${encodeURIComponent(id)}/feynman/check`, {
      method: "POST",
      headers: {
    ...
    return NextResponse.json(response.ok ? data : { error: data.detail ?? "Could not grade this explanation" }, { status: response.status });
  } catch (error) {
