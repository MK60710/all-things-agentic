...
export default function DeepDivePage({ params, searchParams }: { params: Promise<{ paperId: string }>; searchParams: Promise<{ session_id?: string }> }) {
  ...
  return <main className="deep-dive-app">
    <header className="deep-dive-header"><button type="button" onClick={() => router.back()} className="deep-dive-back">← Back to conversation</button><div><small>Deep dive <em>BETA</em></small><h1>{deepDive.title}</h1></div><span>{sectionIndex + 1} of {deepDive.sections.length}</span></header>
